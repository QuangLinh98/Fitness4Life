...
import data.smartdeals_service.dto.blog.ChangePublishedBlogDTO;
import data.smartdeals_service.dto.blog.CreateBlogDTO;
import data.smartdeals_service.dto.blog.UpdateBlogDTO;
import data.smartdeals_service.dto.comment.ChangeStatusCommentDTO;
import data.smartdeals_service.dto.comment.CommentDTO;
...
public class BlogController {
    ...
    public ResponseEntity<?> changeBlogPublishedStatus(
            @PathVariable Long id,
            @Valid @RequestBody ChangePublishedBlogDTO changePublishedBlogDTO,
            BindingResult bindingResult) {
        ...
        try {
            Blog updatedBlog = blogService.changePublishedStatus(id, changePublishedBlogDTO.getIsPublished());
            return ResponseEntity.ok(ApiResponse.success(updatedBlog, "Change blog published status successfully"));
        } catch (Exception ex) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(ApiResponse.errorServer("Server error: " + ex.getMessage()));
        }
